...
export function SettingsPanel() {
  ...
  const builtInKey = hasBuiltInGeminiKey();
    ...
    <div className="space-y-4 overflow-y-auto p-4">
      <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
        <div className="flex items-center gap-2 text-white">
          ...
          <h3 className="text-sm font-semibold">Gemini API</h3>
        </div>
        {builtInKey ? (
          <p className="mt-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-100">
            API key loaded from <code className="text-emerald-200">.env</code> (
            <code className="text-emerald-200">VITE_GEMINI_API_KEY</code>). Rebuild after changing it.
          </p>
        ) : (
          <p className="mt-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
            Add your key to <code className="text-amber-200">.env</code> as{' '}
            <code className="text-amber-200">VITE_GEMINI_API_KEY=AIza…</code> then run{' '}
            <code className="text-amber-200">npm run build</code>. Optional override below.
          </p>
        )}
        ...
        <label className="mt-3 block text-[11px] text-white/45">
          {builtInKey ? 'Optional override (stored in extension sync)' : 'Or paste key here'}
        </label>
        <input
          type="password"
          value={key}
          onChange={(e) => setKey(e.target.value)}
          placeholder={builtInKey ? 'Leave empty to use .env key' : 'AIza… or AQ.…'}
          className="mt-1 w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-white placeholder:text-white/30 focus:border-indigo-400/40 focus:outline-none"
        />
